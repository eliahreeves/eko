...
Deno.serve(async (req) => {
  ...
  const userData = await getUser(record.source_uid);
  ...
  const notifications = devices.map((device: Device) => {
    if (device.notification_type === "apns") {
      return sendAPNS(device.token, record, apnsConfig, title, body);
    } else if (device.notification_type === "web_push") {
      return sendWebPush(device.token, record, title, body);
    }
  });
